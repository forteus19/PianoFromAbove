...
INT WINAPI WinMain( HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, INT nCmdShow )
{
    ...
    g_hWnd = CreateWindowEx( 0, CLASSNAME, L"BetterPFAViz " __DATE__, WS_OVERLAPPEDWINDOW | WS_CLIPCHILDREN, cView.GetMainLeft(), cView.GetMainTop(),
...
DWORD WINAPI GameThread( LPVOID lpParameter )
{
    ...
    wchar_t buf[1024] = {};
    _snwprintf_s(buf, 1024, L"BetterPFAViz %S (Device: %s)", __DATE__, pRenderer->GetAdapterName().c_str());
